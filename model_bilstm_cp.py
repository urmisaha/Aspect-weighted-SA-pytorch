...
print("CP weights - only hidden - upsampled - 2341")

def seed_everything(seed=3412):
    random.seed(seed)
    ...
    for inputs, labels in train_loader:
        ...
        if counter%print_every == 0:
            ...
            if np.mean(val_losses) < valid_loss_min:
                torch.save(model.state_dict(), 'models/state_dict_val_cp_upsample.pt')
                print('Validation loss decreased ({:.6f} --> {:.6f}).  Saving model ...'.format(valid_loss_min,np.mean(val_losses)))
                valid_loss_min = np.mean(val_losses)


# Loading the best model
model.load_state_dict(torch.load('models/state_dict_val_cp_upsample.pt'))
...
print("CP weights - only hidden - upsampled - 3412")
