...
print("randomweighted - only embedding - positive downsampled - 2341")
...
class BiRNN(nn.Module):
    ...
    def forward(self, x, hidden):                               # x.shape =  torch.Size([10, 200]) (batch_size, seq_length)
        ...
        # scores_matrix2 = self.aspect_scores(x)                   # scores_matrix.shape =  torch.Size([10, 200, 1])
    ...
    for inputs, labels in train_loader:
        ...
        if counter%print_every == 0:
            ...
            if np.mean(val_losses) < valid_loss_min:
                torch.save(model.state_dict(), 'models/state_dict_val_randomweighted_downsample2341.pt')
...
trained_scores_dict = {}
f = open('random_trained2341.csv', 'w+')
...
f.close()
pickle.dump(trained_scores_dict, open(f'random_scores_trained_dict2341.pkl', 'wb'))


# Loading the best model
model.load_state_dict(torch.load('models/state_dict_val_randomweighted_downsample2341.pt'))
...
print("num_layers=1 - randomweighted - only embedding - positive downsampled - 2341")
