...
class BiRNN(nn.Module):
    ...
    def __init__(self, vocab_size, target_vocab, output_size, embedding_dim, hidden_dim, n_layers, drop_prob=0.5):
        ...
        # self.attn = nn.Linear(100*200, self.hidden_size * 2)
    ...
    def forward(self, x, hidden):
        ...
        # attn = torch.bmm(hidden, lstm_out)
        # print("attn.size()")
        # print(attn.size())
        # attn = F.softmax(attn)
        # print("after softmax: attn.size()")
        # print(attn.size())
        # new_hidden_state = torch.bmm(attn.permute(0, 1, 2), lstm_out.transpose(1, 2))
        
        # new_hidden_state = new_hidden_state.reshape(x.size(0), 2*hidden_dim)
        ...
        # lstm_out = self.fc(new_hidden_state)
    ...
    for inputs, labels in train_loader:
        ...
        if counter%print_every == 0:
            ...
            if np.mean(val_losses) < valid_loss_min:
                torch.save(model.state_dict(), 'models/state_dict_val_loss_only_emb_pos_downsample.pt')
